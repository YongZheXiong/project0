...
* IEKF / FAST-LIO 核心代码研究和模改仅作为有余力时的后续研究项。如需实际修改核心代码，必须先明确影响范围评估和回滚边界。

### （2）语义导航、任务策略与上层算法边界

* p0_semantic_nav 在项目0中保留迭代一最小语义导航闭环，主要承担语义位置查询和导航目标生成；
* p0_task_manager 保留基础任务生命周期管理和必要扩展空间；
* VLA、世界模型、开放集感知、复杂语义地图、强语义理解、模仿学习和机械臂操作等方向不作为项目0 V2 软件主线，后续由独立机械臂项目或新的公开版本规划承接。

### （3）数据与实验支撑增强

* p0_data_tools 增强实验记录、基线对比与结果导出能力；

因此，迭代二原则上继承既有软件基线，重点在感知层与空间认知层内增强多传感器融合、外挂回环、退化处理和数据实验支撑。语义导航、任务策略、上层算法和强具身智能相关研究不作为 V2 主线；相关受控演进不得削弱安全、人工接管、急停、系统模式仲裁、电源边界、通信主链路和已验收平台基线。
