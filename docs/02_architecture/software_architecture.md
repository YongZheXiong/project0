...
* 项目定义输入 下的项目定义文件集，用于锁定项目整体定位、约束、迭代边界与成果目标；
...
结合项目0的系统定位、项目约束、项目需求、迭代路线与整体成果要求，项目0软件架构在 P1 阶段应同时满足以下目标：
...
迭代二原则上继承迭代一已验收的软件基线与主链路结构，并在不削弱安全、人工接管、急停、系统模式仲裁、电源边界、通信主链路和平台基线的前提下受控演进。

### （1）感知与空间认知增强

* p0_sensor_drivers 增加对 D435 更深入的利用；
* p0_localization 增加多传感器融合增强；
* 必要时增加回环增强相关子模块；如需引入替代、重构或并行实验路线，必须先明确 CR / DEC 支撑、影响范围评估和回滚边界。

### （2）语义导航、任务策略与上层算法增强

* p0_semantic_nav 可增强语义解析能力，并在明确 CR / DEC 支撑下评估更强语义理解或相关研究增强路线；
* p0_task_manager 增强任务表达能力与多轮任务组织能力。
* 开放集感知、复杂语义理解等方向不表示当前已完成事实，也不写成当前阶段必须实现项。

### （3）数据与实验支撑增强

* p0_data_tools 增强实验记录、基线对比与结果导出能力；

因此，迭代二原则上继承既有软件基线，但并不把语义导航、任务策略、上层算法和研究增强模块锁定为只能在现有包内做线性叠加；相关受控演进不得削弱安全、人工接管、急停、系统模式仲裁、电源边界、通信主链路和已验收平台基线。
...
> 项目0在 Orin NX 上采用单一主 ROS2 workspace 作为软件主承载结构，通过 p0_interfaces 统一定义内部接口，通过 p0_bringup 统一组织模式化启动，通过 p0_sensor_drivers、p0_base_bridge、p0_mapping、p0_localization、p0_map_manager、p0_navigation、p0_semantic_nav、p0_task_manager、p0_safety_manager、p0_system_manager、p0_data_tools 等软件包完成系统主链路与辅助链路的软件展开。软件主链路按“感知输入—空间认知—任务决策—行动规划—底盘控制”组织，辅助链路按“安全、管理、记录”组织，并通过架构级话题、服务、动作接口实现跨包解耦；STM32 不运行 ROS2 主业务，仅以独立固件形式通过 p0_base_bridge 接入 ROS2 世界。该软件架构既能直接支撑迭代一最小闭环平台成立，也为后续在 CR / DEC 支撑下对语义导航、任务策略、上层算法和研究增强模块开展受控演进预留空间；此类演进不得削弱安全、人工接管、急停、系统模式仲裁、电源边界、通信主链路和已验收平台基线。
```
