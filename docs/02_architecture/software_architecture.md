...
> **当前实现覆盖（2026-08-20）**：ROS2 包职责与上层接口骨架继续有效，但底盘桥接目标已从旧 STM32 文本协议迁移为 H60 版本化协议和显式 ARM 会话。当前无独立急停/继电器或急停 GPIO。现有源码中的 `ESTOP`、软件急停字段和 `p0_safety_manager` 行为属于旧平台迁移骨架，不得解释为 H60 新车已实现能力，也不得直接用于运动；现行停止/失能语义应迁移为 STOP、DISARM、故障锁止和新会话 ARM。固件要求见 `../../firmware/h60_motion_control_firmware_requirements_v0_2.md`。
...
本文档的目标不是直接替代源代码目录说明、接口字段定义表或 launch 具体实现，而是为迭代一单车系统提供可运行、可联调、可验收的软件基线，以支撑 P2–P6 的底盘控制、传感器接入、定位建图、导航避障、语义导航、任务调度与集成验收。
...
* 项目定义输入，用于锁定项目整体定位、约束、迭代边界与成果目标；
...
* docs/03_design/：承接底盘控制、感知、SLAM、导航、语义导航、任务调度、运行约束、系统管理等模块的详细设计；
...
公开仓库采用 project0/ 作为统一根目录，其中 docs/、firmware/、src/、config/、scripts/、data/、experiments/、simulation/、reports/、presentation/ 等目录共同构成公开工程与成果资料。
...
* data/：rosbag、地图文件、日志、标定数据等运行数据的索引与管理入口；大文件不入 Git，介质分层与保留规则见 `data/README.md`；
...
* p0_manual_control：将经复验的手柄输入转换为带死人开关语义的手动底盘意图；
...
作用：统一承接 ROS2 世界与 STM32 固件世界之间的控制指令下发、状态回传、通信状态监测与桥接转换；旧平台实现将通信新鲜度门限参数化，并对 STM32 四路有符号 16-bit 瞬时计数独立解绕，再进行方向归一化和每轮 counts/rev 换算。

边界：本包只处理“底盘控制与反馈桥接”，不应越级承担任务、语义与导航理解逻辑。轮径 / 轮距未标定前，不由累计轮转数生成轮速、odom、TF 或位姿积分。

## 9.4A p0_manual_control

作用：将手柄 `Joy` 输入转换为带死人开关、失效超时和明确来源的手动底盘指令。

边界：本包不直接发布最终底盘指令，只向 `p0_safety_manager` 提供手动意图；设备映射未确认、死人开关释放、`Joy` 超时或 Flydigi USB 心跳无进度超过 `0.30s` 时必须输出非活动零指令。
...
边界：局部包可以输出自身状态，但停止、失能、人工/自动控制权与恢复条件应由一个运行仲裁点集中处理，避免多个包同时发布相互冲突的运动命令。现有包名 `p0_safety_manager` 是旧迁移标识，不代表产品安全能力。
...
p0_semantic_nav / p0_task_manager
        ↓
p0_safety_manager ← p0_manual_control
        ↓
p0_base_bridge
        ↓
STM32 firmware
...
该链路的意义在于，项目0迭代一不仅要“能跑”，还要形成一个可解释、可管理、可复盘的项目系统。
...
`p0_data_tools` 负责录制、打点、导出、回放辅助和数据索引，不负责把外置介质直接耦合进主业务逻辑。运行时数据先写入 Orin NX 内置 NVMe；停止录制并检查有效性后，再将筛选、裁剪或压缩的关键数据复制到外置机械硬盘。U 盘只承担换机和日常周转，不作为 Project0 原始 rosbag 长期仓库。

存储介质失效、未挂载或复制失败不得改变底盘控制、运行仲裁和 STOP/DISARM 语义。外置机械硬盘只在小车停止后的维护阶段连接，不作为运动中实时录制介质。
...
* 请求响应型接口：主要由 service 承担，用于模式切换、地图保存加载、语义位置查询、STOP/DISARM 等；
...
* p0_navigation 通过数据型接口向 p0_safety_manager 输出自动运动意图；
* p0_manual_control 通过数据型接口向 p0_safety_manager 输出带死人开关的手动运动意图；
* 现有 `p0_safety_manager` 迁移骨架在完成通信、超时、STOP/DISARM 和手动优先仲裁后，向 `p0_base_bridge` 发布唯一最终底盘指令；正式 H60 实现应重新确认包名与接口语义；
...
这与项目0“先完成固定场景单车闭环，再谈有限增强”的顺序一致。

## 18.2 迭代二阶段

迭代二原则上继承迭代一已验收的软件基线与主链路结构，并在不削弱 STOP/DISARM、通信超时停车、人工接管、系统模式仲裁、电源边界和通信主链路的前提下受控演进。

### （1）多传感器融合与空间认知增强

* p0_sensor_drivers 增加对 D435、编码器里程计和相关时间同步 / 状态观测的支撑；
* p0_localization 增加多传感器融合增强，优先服务定位建图鲁棒性、退化场景处理和导航稳定性；
* 必要时增加外挂回环、地图一致性修正或退化处理相关子模块；外挂回环优先作为 FAST-LIO2 基线之外的可回退增强，不在一开始修改 FAST-LIO2 核心；
* IEKF / FAST-LIO 核心代码研究和模改仅作为有余力时的后续研究项。如需实际修改核心代码，必须先明确 CR / DEC 支撑、影响范围评估和回滚边界。

### （2）语义导航、任务策略与上层算法边界

* p0_semantic_nav 在项目0中保留迭代一最小语义导航闭环，主要承担语义位置查询和导航目标生成；
* p0_task_manager 保留基础任务生命周期管理和必要扩展空间；
* VLA、世界模型、开放集感知、复杂语义地图、强语义理解、模仿学习和机械臂操作等方向不作为项目0 V2 软件主线，后续由独立机械臂项目或新的 CR / DEC 承接。

### （3）数据与实验支撑增强

* p0_data_tools 增强实验记录、基线对比与结果导出能力；

因此，迭代二原则上继承既有软件基线，重点在感知层与空间认知层内增强多传感器融合、外挂回环、退化处理和数据实验支撑。语义导航、任务策略、上层算法和强具身智能相关研究不作为 V2 主线；相关演进不得削弱 STOP/DISARM、通信超时停车、人工接管、系统模式仲裁、电源边界和已验收的单车基线。
...
> 项目0在 Orin NX 上采用单一 ROS2 workspace，通过 `p0_interfaces` 统一内部接口、`p0_bringup` 组织模式启动，并由传感器、底盘桥接、建图定位、导航、任务、运行仲裁、系统管理和记录包完成主辅链路。H60 不运行 ROS2 主业务，以运动控制固件通过 `p0_base_bridge` 接入；桥接必须实现版本化协议、STOP/DISARM、新会话显式 ARM、输入回中、超时停车和故障状态。现有旧 STM32 文本桥只作迁移骨架，完成 H60 无动力与故障注入验收前不得用于动力控制。
