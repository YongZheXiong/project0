...
3. 不接电机、电驱或主电池；
4. 不接 Orin NX UART；
5. 底盘控制、ROS2-STM32 通信和电气联调均不在本节完成范围内。
...
4. 不写入 Flash；
5. 不执行调试运行；
6. 本节结束时 STM32 仍未进入下载、烧录、运行或底盘控制联调状态。
...
5. STM32CubeProgrammer 的 ST-LINK 通道当前不是可用下载链路；
6. `No ST-LINK...` 记录为工具链边界，现有证据不足以判定板卡故障。
...
替换板当时需要按单板供电、车载 5V、SWD、USART3 和 Orin NX 收包的顺序重新验收。技术摘要见 `../../hardware/archive/wiring_legacy_pre_h60_2026-08-20/p2_stm32_power_fault_summary_2026-06-17.md`。
...
2. 正式底盘控制协议、通信中断停车、无新指令停车、编码器方向符号归一化和里程计当时仍未完成；
