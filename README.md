...
| 项目 | 状态 |
| --- | --- |
| 当前阶段 | P1.4：工程环境搭建已启动；P1.3 到货验收与 P2 装配前置并行承接 |
| 当前目标 | 基于 Orin NX 实机基线推进 ROS2 工程环境搭建，同时继续准备后续机械 / 电气装配验证 |
| ROS2 基线 | ROS2 Humble 基础环境已完成最小 talker/listener 通信验证 |
...
当前状态不表示整车已经完成装配、接线、上电或验收。硬件和部署记录会随项目推进继续更新。
...
- `docs/04_deployment/`：Orin NX 基线、ROS2 环境和后续部署记录。
...
后续会继续补充 ROS2 工作区、底层通信、传感器驱动、SLAM、导航和系统启动流程。
