...
截至 2026-09-01，项目处于 H60 + MC520 底盘重建阶段。Orin NX/C1902 在一次电气异常后已停止使用并送交专业维修；维修期间项目临时只推进 H60 能独立承接的基础运动子轨。

| 项目 | 当前状态 |
| --- | --- |
| 阶段与版本 | P2 / `v1.1-dev` / Unreleased；最新开发预发布 `v1.1-dev.2` |
...
| 固件 | H60 `v0.1.1` 运动锁定固件及主机测试已发布；当前仍为 `MOTION=0`，PWM、编码器闭环和运动门未开放 |
| ROS2 | 已发布 H60 二进制协议桥、手柄输入与安全仲裁的离线基线；Orin 不可用期间不声称完成 ROS2/实机联调 |
...
当前仓库提供 H60 架构、接线、运动锁定固件、二进制协议桥、CAD 和 H0-H8 验收基线。已发布的软件只覆盖离线/无动力安全基线；经 H5-H7 验收的动力运行固件仍在开发中。
...
- 最新开发预发布：`v1.1-dev.2`（2026-09-01）；它冻结 H60 安全固件/协议桥与临时执行边界，不表示 P2、基础运动或手柄控制已完成。
