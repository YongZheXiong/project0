...
截至 2026-08-19，项目采用 **OpenCTR H60 V3.7 + 4 × MC520P56_12V + 65 mm 橡胶轮** 的重建数字基线。当前两层整车的 28 个 STEP 已完成第二轮数字复审；此前 STM32 + WSDC2412D + JGB37-520 + 三层底盘属于上一版实车，相关记录仅用于历史追溯。

| 项目 | 当前口径 |
| --- | --- |
| 阶段 | P2 重建：当前 CAD 数字复审完成；两层铝板、D435 支架和 Mid360 平台已委托加工，待交付、试装和实物验收 |
...
| 结构 | 轴距 170 mm、轮距约 184.16 mm、6 根层间支柱、D435 水平安装；28 个 STEP 全部 B-Rep 有效 |

“已委托加工”不表示制件已交付或合格。当前状态也不表示整车已经接线、上电或具备落地运动条件。

公开接线和急停条目仍反映现有安全设计基线；CAD 中未建模独立急停/继电器，二者尚待受控统一，不能根据 CAD 缺件推导安全门已取消或已通过。
...
加工件收货与不上电试装 → 质量/重心与维护检查 → 断电接线检查 → 独立低功率上电 → 安全链和反供电验证 → 安全固件 → 单路/四路悬空动力 → 落地低速。
