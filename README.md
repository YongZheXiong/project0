...
截至 2026-09-08，项目处于 H60 + MC520 底盘重建阶段。Orin NX/C1902 在一次电气异常后已停止使用并送交专业维修；维修期间项目只推进 H60 能独立承接的基础运动子轨。

| 项目 | 当前状态 |
| --- | --- |
| 阶段与版本 | P2 / `v1.1-dev` / Unreleased；阶段性成果版本 `v1.1.1` 已冻结四轮单轮受限表征，P2 本身尚未完成 |
...
| 阶段性成果 | 四个轮位已有单轮双向物理方向、编码器符号、两次五圈工程 CPR 和受限停车证据；后轮使用临时 20 cm 原厂线，最终线束与四轮组合仍待验证 |
...
当前仓库提供 H60 架构、接线、默认运动锁定固件、受显式构建门保护的运动/单通道校准源码、二进制协议桥、CAD 和 H0-H8 验收基线。`v1.1.1` 额外冻结四轮单轮受限表征；经 H5-H7 验收的动力运行固件仍在开发中。
...
- 当前开发版本：`v1.1-dev`；已发布的首个阶段性成果版本为 `v1.1.1`。P2 完成并通过发布审查后才进入正式 `v1.1`。
- 最新阶段性成果：`v1.1.1`（2026-09-08），冻结四轮单轮运动方向、编码器符号、工程 CPR 与受限停车证据，不表示 P2 或四轮运动验收完成。
