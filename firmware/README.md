...
| `stm32_uart3_minimal_test_v0_1.md` | 记录 USART3 固定文本最小发送方案和当前阻塞状态 | 待替换板验收后执行 |
...
7. 原 STM32 控制板随后发生 3.3V 供电故障，当前不能确认其仍在运行任何固件；
8. USART3 最小测试等待替换板完成供电与 SWD 验收后继续；
9. 正式底盘控制固件仍待后续建立，不应把安全初始化记录或临时测试记录等同于正式控制固件。
